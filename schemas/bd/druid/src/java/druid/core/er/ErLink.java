...
public class ErLink
{
	...
	public ErScrEntity getStartEntity() { return scrEntStart; }
	public ErScrEntity getEndEntity()   { return scrEntEnd;   }
	...
	public int getDistanceFromPoint(int x, int y)
	{
		...
		//--- t is the parameter of the point P = P0 + t(P1-P0)
		//--- Q is the point (x,y) given in input
		//--- we want to calculate the minimum distance PQ
		//--- the following formula calculates the t value that gives such P

		float t = ((x - x0)*dx + (y - y0)*dy) / (dx*dx + dy*dy);

		//--- check if the distance is outside the boundary P0-P1

		if (t < 0 || t > 1)
			return -1;

		//--- calculates such point P

		float px = x0 + t * dx;
		float py = y0 + t * dy;

		//--- return the minimun distance (int) from the given point to the line
	...
	public void draw(Graphics g)
	{
		...
		//--- first: calc the line length (in pixel)
		...
		//--- second: if the two entities are too near --> skip drawing
		...
		//--- calc intersections between the line and the two entities
		//--- this step is mandatory because we have to draw the images
		//--- at the two end of the line
		...
		//--- calc deltas for starting image
		...
		{
			//--- ok guys, we must draw a dashed line. The stroke facility of
			//--- Graphics2D is too slow so we can draw the line by hand as
			//--- fast as we can...
			//--- calc how many segments need to be drawn
			//--- we want each segment of 8 pixel length so we must divide
			//--- by 16 (8 pixel line + 8 pixel empty)
	...
	//---------------------------------------------------------------------------
	//--- calculate intersetion of (desX,desY) with rectangle r (with center
	//--- (srcX,srcY)). Return value into p
	//--- the intersection point is located outside the rectangle r

	private void calcIntersection(Rectangle r, int srcX, int srcY,
											int desX, int desY, IntersPoint p)
	{
		//------------------------------------------------------------------------
		//--- case 1
