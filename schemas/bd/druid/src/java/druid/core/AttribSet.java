...
public class AttribSet
{
	...
	//---------------------------------------------------------------------------
	protected AttribSet getNewInstance() { return new AttribSet(); }
	//---------------------------------------------------------------------------
	...
	{
		if (!htAttribs.containsKey(name))
			throw new DruidException(DruidException.ILL_ARG, "Attrib not found", name);
	...
	{
		//--- stage 1 : collect all unused attribs
		...
		for(Enumeration e=htAttribs.keys(); e.hasMoreElements();)
		{
			...
			if (!as.contains(attrib))
				v.add(attrib);
		}

		//--- stage 2 : remove all unused attribs

		for(int i=0; i<v.size(); i++)
			htAttribs.remove(v.get(i));

		//--- stage 3 : add remaining attribs

		for(Enumeration e=as.attribs(); e.hasMoreElements();)
		{
			String attrib = (String) e.nextElement();

			if (!htAttribs.containsKey(attrib))
				htAttribs.put(attrib, as.getData(attrib));
		}
	}
	...
	{
		if (htAttribs.containsKey(name))
			throw new DruidException(DruidException.ILL_ARG, "Attrib already in use", name);
...
class Attrib
{
	...
	//---------------------------------------------------------------------------
	//--- used to convert an attrib's value read from disk
	...
	public void setData(Object data)
	{
		...
		if (bStr || bInt || bBool)
			throw new DruidException(DruidException.ILL_ARG, "Wrong attrib class", data);

		if (!oData.toString().equals(data.toString()))
	...
	//---------------------------------------------------------------------------
	//--- used to save attrib's value to disk
