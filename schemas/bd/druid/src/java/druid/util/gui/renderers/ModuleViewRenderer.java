...
public class ModuleViewRenderer extends DefaultTreeCellRenderer
{
								...
								int row, boolean hasFocus)
	{
		...
		if (o instanceof BasicModule)
			setIcon(ImageFactory.MODULE);

		//--- root node used to store modules config

		else if (o instanceof DatabaseNode) {}
		...
		else
		{
			...
			if (modClass.equals(DATAGEN))
				setIcon(ImageFactory.PAGE);

			else if (modClass.equals(DATABASE))
				setIcon(ImageFactory.DATABASE);

			else if (modClass.equals(JDBC))
				setIcon(ImageFactory.GEAR);

			else if (modClass.equals(TREENODE))
				setIcon(ImageFactory.POPUP);
