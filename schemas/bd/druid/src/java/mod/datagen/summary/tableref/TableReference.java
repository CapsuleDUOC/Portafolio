...
public class TableReference implements SummaryGenModule
{
	...
	public String getId()       { return "tableRef"; }
	...
	public String  getFormat()        { return "Table references"; }
	...
	public void generate(Logger logger, DatabaseNode dbNode)
	{
		...
		try
		{
			...
			for(int i=0; i<tables.size(); i++)
				genTableReference(w, (TableNode)tables.get(i), tables);
	...
	private void genTableReference(Writer w, TableNode tableNode, List<TableNode> tables) throws IOException
	{
		...
		for(TableNode otherTableNode : tables)
			for(int i=0; i<otherTableNode.getChildCount(); i++)
			{
					...
					if (tableNode == dbaseNode.getTableByID(tableId))
					{
						...
						else
						{
							...
							if (!onUpdate.equals(FieldNode.NOACTION))
							{
								...
								if (!onDelete.equals(FieldNode.NOACTION))
									w.write(" | ");
							}
							...
							if (!onDelete.equals(FieldNode.NOACTION))
								w.write("on delete "+ onClauseDec.decode(onDelete));
