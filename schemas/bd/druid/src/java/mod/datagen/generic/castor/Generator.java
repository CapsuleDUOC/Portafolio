...
public class Generator
{
	...
	public XmlElement generate()
	{
		...
		for(Enumeration e=dbNode.preorderEnumeration(); e.hasMoreElements();)
		{
			...
			if (node instanceof TableNode)
				elRoot.addChild(genClass(mod, (TableNode) node));
		}

		//--- add 'key-generator' elements
	...
	private XmlElement genClass(BasicModule mod, TableNode node)
	{
		...
		if (sett.isAutoComplete())
			elTable.setAttribute(new XmlAttribute("auto-complete", sett.isAutoComplete()));

		if (!sett.isVerifyConstr())
			elTable.setAttribute(new XmlAttribute("verify-constructable", sett.isVerifyConstr()));

		if (!sett.getAccess().equals(TableSettings.ACCESS_SHARED))
			elTable.setAttribute(new XmlAttribute("access", sett.getAccess()));

		if (!sett.getKeyGen().equals(TableSettings.KEYGEN_NONE))
			elTable.setAttribute(new XmlAttribute("key-generator", sett.getKeyGen()));

		//--- 'description' subelement

		addDescr(elTable, node);

		//--- 'cache-type' subelement
		...
		if (!sett.getCacheType().equals(TableSettings.CACHETYPE_COUNTLIM) || !sett.getCacheCap().equals(""))
		{
			...
			if (!sett.getCacheType().equals(TableSettings.CACHETYPE_COUNTLIM))
				elCache.setAttribute(new XmlAttribute("type", sett.getCacheType()));

			if (!sett.getCacheCap().equals(""))
				elCache.setAttribute(new XmlAttribute("capacity", sett.getCacheCap()));
		...
		//--- generate fields and collect data for 'identity' attribute
		...
		for(int i=0; i<node.getChildCount(); i++)
		{
			...
			if (DataLib.isPrimaryKey(field))
				v.add(field.attrSet.getString("name"));
		...
		if (v.size() != 0)
			elTable.setAttribute(new XmlAttribute("identity", v.toString()));
	...
	private XmlElement genField(BasicModule mod, FieldNode node)
	{
		...
		if (sett.isDirect())
			elField.setAttribute(new XmlAttribute("direct", sett.isDirect()));

		if (sett.isLazy())
			elField.setAttribute(new XmlAttribute("lazy", sett.isLazy()));

		if (!sett.getHandler().equals(""))
			elField.setAttribute(new XmlAttribute("handler", sett.getHandler()));

		if (!sett.getGetMethod().equals(""))
			elField.setAttribute(new XmlAttribute("get-method", sett.getGetMethod()));

		if (!sett.getSetMethod().equals(""))
			elField.setAttribute(new XmlAttribute("set-method", sett.getSetMethod()));

		if (!sett.getHasMethod().equals(""))
			elField.setAttribute(new XmlAttribute("has-method", sett.getHasMethod()));

		if (!sett.getCreateMethod().equals(""))
			elField.setAttribute(new XmlAttribute("create-method", sett.getCreateMethod()));

		if (sett.isTransient())
			elField.setAttribute(new XmlAttribute("transient", sett.isTransient()));

		if (!sett.getContainer().equals(FieldSettings.OMIT))
			elField.setAttribute(new XmlAttribute("container", sett.getContainer()));

		if (!sett.getCollection().equals(FieldSettings.COLL_NONE))
			elField.setAttribute(new XmlAttribute("collection", sett.getCollection()));
		...
		if (node.isFkey())
		{
			...
			//--- table and field must exist because we do the integrity check
		...
		if (sett.isSqlReadOnly())
			elSql.setAttribute(new XmlAttribute("read-only", sett.isSqlReadOnly()));

		if (!sett.getSqlDirty().equals(FieldSettings.DIRTY_CHECK))
			elSql.setAttribute(new XmlAttribute("dirty", sett.getSqlDirty()));

		if (sett.isSqlTransient())
			elSql.setAttribute(new XmlAttribute("transient", sett.isSqlTransient()));
	...
	private void genKeyGens(XmlElement el)
	{
		...
		if (!dbSett.getHLTable().equals(""))
			setParam(elKeyGen, "table", dbSett.getHLTable());

		if (!dbSett.getHLKeyField().equals(""))
			setParam(elKeyGen, "key-column", dbSett.getHLKeyField());

		if (!dbSett.getHLValueField().equals(""))
			setParam(elKeyGen, "value-column", dbSett.getHLValueField());

		if (!dbSett.getHLGrabSize().equals(""))
			setParam(elKeyGen, "grab-size", dbSett.getHLGrabSize());

		if (dbSett.isHLSameConn())
			setParam(elKeyGen, "same-connection", dbSett.isHLSameConn());

		if (dbSett.isHLSameConn())
			setParam(elKeyGen, "global", dbSett.isHLGlobal());
		...
		if (!dbSett.getSEQSequence().equals(""))
			setParam(elKeyGen, "sequence", dbSett.getSEQSequence());

		if (!dbSett.getSEQIncrement().equals(""))
			setParam(elKeyGen, "increment", dbSett.getSEQIncrement());

		if (dbSett.isSEQReturning())
			setParam(elKeyGen, "returning", dbSett.isSEQReturning());

		if (dbSett.isSEQTrigger())
			setParam(elKeyGen, "trigger", dbSett.isSEQTrigger());
	...
	private void addDescr(XmlElement el, AbstractNode node)
	{
		...
		if (!descr.equals(""))
			el.addChild(new XmlElement("description", descr));
	}
...
//		if ("int4".equals(ti.basicType)) type = "integer";
//		if ("int8".equals(ti.basicType)) type = "long";
//		if ("text".equals(ti.basicType)) type = "varchar";
