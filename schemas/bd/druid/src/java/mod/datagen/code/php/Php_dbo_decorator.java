...
/**Generates DBO specific PHP code for CRUD capable model classes based on Druid Table definitions.
 * @author mole
 *
 */
public class Php_dbo_decorator extends Php implements Php_crud_generator {
	...
	public Php_dbo_decorator(TableNode node, Php parent) throws WrongDbTypeException {
		if (sett.getNameDbType().equals(Settings.TYPE_DB_PDO)==false) {
	...
	public StringBuffer getLoadMethod() { 
		...
		if (numberOfPrimaryKeys==0) {
			...
			if (parent.sett.getNameDbType().equals(Settings.TYPE_DB_MYSQLI)) {
				...
				for (int i = 0; i < node.getChildCount(); i++) {
					...
					if(DataLib.isPrimaryKey(f)) { 
						...
						typeList = typeList + parent.getMySQLIType(f);
						comma = ", ";
					}
				} 
			...
			if (parent.sett.getNameDbType().equals(Settings.TYPE_DB_PDO)) {
	...
	public StringBuffer getLoadArrayKeyValueSearchMethod() {
		...
		w.append(parent.getSmallSeparator());
