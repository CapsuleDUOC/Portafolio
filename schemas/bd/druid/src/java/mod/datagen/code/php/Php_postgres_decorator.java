...
/**Generates Postgresql specific PHP code for CRUD capable model classes based on Druid Table definitions.
 * @author mole
 *
 */
public class Php_postgres_decorator extends Php implements Php_crud_generator {
	...
	public Php_postgres_decorator(TableNode node, Php parent) throws WrongDbTypeException {
		if (sett.getNameDbType().equals(Settings.TYPE_DB_POSTGRES)==false) {
	...
	public StringBuffer getLoadArrayByFullTextMethods() {
		...
		if (hasFullText && sett.getNameDbType().equals(Settings.TYPE_DB_MYSQL)) { 
